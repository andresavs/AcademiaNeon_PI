pipeline {
    ...
    environment {
        ...
        BUCKET_NAME="digitalhouse_devopers_homolog"
        REGION="us-east-1" 
        PERMISSION=""
                ...
                stage('Test image') {
                    steps {
                        script {
                            ...
                            docker.image("digitalhouse-devops:latest").withRun('-p 3000:3000') { c ->
                                sh 'docker ps'
                                sh 'sleep 10'
            ...
            steps { 
                script {
                    if(env.GIT_BRANCH=='origin/master'){
                        ...
                        sh "docker run -d --name app_homolog -p 3000:3000 933273154934.dkr.ecr.us-east-1.amazonaws.com/digitalhouse-devops:latest"
                        sh "docker ps"
                        sh 'sleep 10'
                        sh 'curl http://127.0.0.1:3000/api/v1/healthcheck'
                ...
                script {
                    if(env.GIT_BRANCH=='origin/master'){
                        ...
                        environment {
                            ...
                            AWS_SECRET_ACCESS_KEY=""
                            AWS_SDK_LOAD_CONFIG="0"
                            BUCKET_NAME="digitalhouse_devopers_producao"
                            REGION="us-east-1" 
                            PERMISSION=""
                            ACCEPTED_FILE_FORMATS_ARRAY=""
                        }
                        ...
                        sh "docker run -d --name app_prod -p 80:3000 933273154934.dkr.ecr.us-east-1.amazonaws.com/digitalhouse-devops:latest"
                        sh "docker ps"
                        sh 'sleep 10'
